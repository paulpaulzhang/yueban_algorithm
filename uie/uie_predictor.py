...
class CoreMLInferBackend(object):
    def __init__(self,
                 model_path_prefix):
        import coremltools as ct
        logger.info(">>> [CoreMLInferBackend] Creating Engine ...")
        mlmodel = os.path.join(
            model_path_prefix, "UIE.mlpackage")
        if not os.path.exists(mlmodel):
            raise OSError(f'{mlmodel} not exists!')
        infer_model_dir = model_path_prefix

        self.mlmodel = ct.models.MLModel(
            mlmodel, compute_units=ct.ComputeUnit.CPU_ONLY)
        logger.info(">>> [InferBackend] Engine Created ...")
    ...
    def infer(self, input_dict: dict):
        ...
        predict = self.mlmodel.predict(input_dict)
        start, end = predict['start_prob'], predict['end_prob']
        return start, end
...
class UIEPredictor(object):
    ...
    def _prepare_predictor(self):
        assert self._engine in ['pytorch',
                                'onnx', 'coreml'], "engine must be pytorch or onnx!"
        ...
        if self._engine == "coreml":
            self.inference_backend = CoreMLInferBackend(self._task_path)
